...
async function sendToApns(params: {
  ...
  let okCount = 0;
  const failures: Array<{ token: string; status: number; body: string; reason: string | null }> = [];
  ...
  for (const batch of batches) {
    const results = await Promise.all(batch.map(async (token) => {
      ...
      if (res.ok) {
        return { token, ok: true as const, status: res.status, body: "", reason: null as string | null };
      }
  ...
  return { okCount, failures, invalidTokens };
}
...
Deno.serve(async (req) => {
  try {
    ...
    const sampleFailures: Array<{ env: string; token: string; status: number; reason: string | null; body: string }> = [];
    ...
    for (const env of envs) {
      ...
      const { okCount, failures, invalidTokens } = await sendToApns({
        host,
        tokens,
        jwt,
