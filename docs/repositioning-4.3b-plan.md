...
- [x] **RP-03 · Home hero reframe — DONE 2026-07-23** («середній» варіант за рішенням owner).
  `.ritual-band` = primary CTA (однорядкова: 4 крапки прогресу · адаптивний лейбл;
  тап → перший незроблений крок, reflection-first → /journal). Після 3 ітерацій owner
  зафіксував розміщення: **док над нижнім меню** (під focus-today); шапка/астро/hero —
  на оригінальній геометрії. Рядок неба в стрічці заборонений (дублює астро-картку). Таро-коло лишилось, але
...
- [x] **RP-04 · Onboarding reframe — DONE 2026-07-23.** Сабтайтл онбордингу: «Tarot and
  ...
  **Перший запуск → журнал:** чистий запуск (guard стемпить `from=/`) після онбордингу
  веде на `/journal?source=onboarding&entry=first_run` — перша хвилина в апці = ритуал
  рефлексії (ключовий аргумент для рев'юера). Дип-лінки з валідним `from` повертаються
  на своє місце (N2 збережено); `/` прибрано з вайтлиста свідомо.
- [x] **RP-11 · Numerology engine input — DONE 2026-07-23.** Число дня фідить банк промптів
  ...
  число дня для гостей (рахується чисто з дати — працює офлайн). Маленький підпис у
  sky-рядку журналу: «День 4» / «Особистий день 4». Ніяких згадок у назві/сабтайтлі/
  скріншотах (обмеження діє). Повний екран Numbers — v1.2.
