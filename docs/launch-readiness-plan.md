...
- [~] **LR-12 — On-device sandbox checks — MOSTLY DONE 2026-07-07/08.** ✅ Restore verified (delete app → TestFlight reinstall → Restore returned the still-active sub — legit: TestFlight yearly auto-renews daily, RENEWAL #8 seen in `user_entitlements`). ✅ `VITE_RC_IOS_API_KEY` confirmed in `.env` (removed a shadowing empty duplicate line). **Remaining tonight (~21:34 Kyiv 2026-07-08):** sub expires → verify EXPIRATION webhook lands + premium revokes on device + negative Restore (no unlock). Note: Cancel-via-Settings isn't reachable on TestFlight builds (no Sandbox Account section) — covered by natural expiration instead.
- [x] **LR-13 — Subscriptions "Ready to Submit" in ASC — DONE 2026-07-07.** Both subs Ready to Submit (localizations en+uk, group localization, review screenshot from device, both moved to Level 1) and attached to the version page.
- [x] **LR-14 — Screenshots — DONE 2026-07-08 (upgraded to 8-screen set).** Owner review found tarot/compatibility shots empty → generator reworked (commit `81f4c68`): revealed 3-card spread, real compat result (71 MATCH), + daily-card / card-library / zodiac-guide screens; premium shows real $29.99 (was "Purchases are unavailable" — fixed via DEV-only `?qa=premium` seed, commit `c8583f4`). Uploaded to ASC: 6.9" + 6.5" × 8; smaller sizes intentionally left to Apple's auto-scaling.
- [x] **LR-16 — Age Rating + App Privacy — DONE (verified 2026-07-08).** App Privacy was already published (9 data types, matches manifest 1:1, Linked/no-Tracking). Age rating 4+.
- [x] **ASO/metadata pass — DONE 2026-07-08.** Subtitle set (`Daily card & zodiac ritual`); keywords fixed (dropped dishonest `numerology`, added `love`; no title/subtitle word duplication); description/promo verified (3.1.2 sub terms + EULA + privacy links). **Review-friction pack:** demo video attached to App Review Information + Notes rewritten (differentiation vs 4.3 template-app risk; account-before-purchase justification vs 5.1.1).
...
#### LR-26 · Daily-horoscope premium `detailed` not enforced at source — P2 (server, needs deploy)
- **Status:** [ ] PARKED post-launch — re-verified & decided **2026-07-04** (do NOT fix pre-launch; low risk, don't destabilize a working load path).
- **What:** the daily-wheel horoscope loads via a direct PostgREST select on `horoscopes` (`select=…,detailed`, all themes, **no entitlement filter**, no RLS) — `supabaseNativeCore.js:254`. So premium `love`/`career` `detailed` text ships in the **raw network payload** to any authenticated user.
- **Why it's low severity (mobile):** no UI leak (locked themes are blurred, premium text never enters the DOM), cache is entitlement-safe (A-4 fixed), and the "leak" only reaches someone who deliberately proxies HTTPS / hits REST with the (already-public) anon key. What leaks is a **pre-generated horoscope paragraph** — not money, not user data, and not per-request AI cost (it's batch content). The money surfaces (tarot AI, compatibility AI, personal-horoscope) **are** server-enforced 403 with `RC_ENFORCE_PREMIUM=true` (verified live).
- **Fix (post-launch):** withhold premium `detailed` at source — entitlement-aware read (RLS premium-detail split, `SECURITY DEFINER` view, or route the read through an authed edge fn using `_shared/premium.ts isUserPremium()`). Client already tolerates missing premium `detailed` (renders the lock). Same item logged under the 2026-06-25/28 daily-log lines.
...
- 2026-07-08 — **App-Store reviewer simulation (5-agent audit → verdict ~82% approve, 0 blockers) + SHOULD-FIX sweep.** Fixed all 4 code items from the audit: (1) Apple Sign In **Cancel is no longer shown as an error** (new `src/helpers/appleSignInError.js` `isAppleSignInCancel` — plugin rejects with localizedDescription only, detect `error 1001`/cancel/скасовано; LoginView + SignUpScene; unit test `tests/appleSignInError.test.js`); (2) SavedReadings "Log in" now carries `redirect: '/readings'` (+ source-contract test `tests/savedReadingsLoginRedirect.test.js` — the rendering branch is premium-flag-without-session, revoked on boot, so not honestly reachable in Playwright); (3) PersonalHoroscope: failed Regenerate no longer hides the existing reading (reading wins over error; error shows as `.personal-error-banner` above it); (4) horoscope `.horoscope-lock-overlay` `z-index:2` — skeleton no longer paints over the "Unlock Premium" CTA (verified via QA screenshot of `?theme=love`). `app-store/metadata.md` marked **DEPRECATED** (kept only for the ai-ops scanner; canonical = `asc-metadata.md`). Tests **279/279**, Playwright **30/30**, eslint 0. **Evening additions:** (5) user-reported bug — login → /premium → Close did history-back into /login; guard now bounces an authenticated user off `login`/`signUp` to home (`src/router/guard.js`, deliberately ignoring the stale `redirect` query to avoid a close-loop; +2 unit tests in `routerGuard.test.js`, tests **281/281**); (6) LR-12 expiration check re-read: premium alive on device is CORRECT — prod `user_entitlements` shows **RENEWAL #9 at 21:40 Kyiv** (TestFlight yearly renews daily; natural EXPIRATION lands ~renewal #12 in ~3 days). Decision: don't block submission on it — the expiration→revoke chain is already verified (webhook smoke 06-24, ordering migration 06-28, resume-sync LR-09); watch the webhook when it fires naturally; negative-Restore still worth doing with a second sandbox account. **Pending (yours):** delete junk `ios/App/Pods/**/Frameworks 2/` dirs (`rm -rf "ios/App/Pods/GoogleAppMeasurement/Frameworks 2" "ios/App/Pods/FirebaseAnalytics/Frameworks 2"` — contains the unlinked IDFA IdentitySupport variant, hygiene/footgun); add the AI-generation paragraph to App Review Notes in ASC; tonight's EXPIRATION sandbox check.

- 2026-07-07/08 — **ASC submission prep session (LR-12/13/14/16 + ASO).** LR-13 done (subs Ready to Submit, Level 1, attached to version). LR-14 done+upgraded: 8-screen live-state set (3-card tarot spread, revealed compat result, daily card, card library, zodiac guide; premium price fix `c8583f4`, set rework `81f4c68`) uploaded 6.9"+6.5". LR-16 verified done (App Privacy was already published, 9 types match manifest; age 4+). Subtitle+keywords fixed; demo video + rewritten Review Notes attached (4.3/5.1.1 mitigation). LR-12: Restore verified via TestFlight reinstall (sub still auto-renewing, RENEWAL #8); `.env` RC key dedup; expiration + negative-restore tests scheduled for tonight (~21:34 Kyiv). Final gate: tests 276/276, eslint 0 (2 legacy test-file fixes `d43ebdd`), Playwright 18/18, build OK. **Next: tonight's expiration check → Add for Review.**
...
- 2026-07-04 — **Full free-vs-premium honesty audit (4-agent code trace) + compatibility re-gate.** Triggered by a real-user report that premium compatibility wasn't worth paying for. Re-gated Compatibility (option A): free = overall score + tier + single strongest sphere; premium = all spheres scored/explained + AI dynamic/advice + weekly weather + houses (rewrote misleading "preview" copy en/uk, updated `premium-matrix.md` + copy-consistency test; commit `d819126` + defensive `hasPremiumAccess &&` on `compat-ai-extra`). Then audited **all** premium claims vs actual code: **Tarot** (1 session/day, 1-vs-3/5 cards, real AI depth), **Horoscope** (energy-only free, love/career blurred, A-4 cache fixed), **Compatibility** (re-gate correct, leak-free, server-enforced 403), **History** (gated both save+read). **All UI claims MATCH reality.** Verified `RC_ENFORCE_PREMIUM=true` live (digest = sha256("true")) → money surfaces server-enforced. **Only gap = LR-26** (daily-horoscope `detailed` in raw REST payload) — re-confirmed low-risk for mobile, **parked P2** (don't destabilize a working load path pre-launch). tests 250/250, build OK. All local (not pushed).
- 2026-06-28 — **Premium check made authoritative (no "paid → blocked" 403).** Root-caused a live incident: AI tarot returned 403 `premium_required` for a real premium user because `RC_ENFORCE_PREMIUM=true` was on while `user_entitlements` wasn't back-filled (webhook not configured) — exactly the rollout caveat. Immediate fix: set `RC_ENFORCE_PREMIUM=false` (AI works for all logged-in). Structural fix: `_shared/premium.ts` `isUserPremium()` now does cache → **RevenueCat REST authoritative check** (`GET /v1/subscribers/{user_id}` with `RC_SECRET_API_KEY`), so a just-purchased user is recognized instantly without waiting on the webhook; **fail-open only on an RC API outage** (never block a payer; AI features aren't sensitive data). Premium stays protected — a genuine non-payer still gets RC's authoritative "no". `node --check` clean; Deno not run locally.
  - **DONE & LIVE (2026-06-28):** `RC_SECRET_API_KEY` set, the 3 functions redeployed, `RC_ENFORCE_PREMIUM=true` — verified: a premium account gets real AI, the authoritative RC check recognizes it. **IMPORTANT:** the key MUST be a **v1 (legacy) secret key** — a v2 secret key returns error `7723` ("incompatible with API V1") because `_shared/premium.ts` calls the `/v1/subscribers` endpoint. If you ever rotate it, use a v1 key (or move the code to the v2 API). Webhook (#26/#27) + back-fill are now just a cache optimization, not a correctness dependency.
