...
const mergeSession = (existing, refreshed) => {
  ...
  if (!next.expires_at && next.expires_in) {
    next.expires_at = Math.floor(Date.now() / 1000) + Number(next.expires_in || 0)
  }
  return next
}
...
const requestJson = async (url, init, timeoutMs, label) => {
  ...
  const doFetch = useNative ? nativeFetch : fetch
  // silent
...
export const refreshAccessTokenNative = async (timeoutMs = 8000) => {
...
export const getUserNative = async (timeoutMs = 6000) => {
  const stored = await readStoredSession()
  if (stored?.user) {
...
export const setSessionFromTokens = async ({ access_token, refresh_token, expires_in, expires_at }) => {
...
export const updateUserPasswordNative = async (password, timeoutMs = 8000) => {
  if (!supabaseUrl) return { data: null, error: new Error('Supabase URL missing') }
  const makeRequest = async () => {
...
export const selectAppUser = async (userId, timeoutMs = 6000, fields = 'name,email,date_of_birth') => {
...
export const selectTarotReadingsByUser = async (userId, timeoutMs = 8000) => {
  if (!restBase) return { data: null, error: new Error('Supabase URL missing') }
  const makeRequest = async () => {
    const headers = await buildAuthHeaders()
...
export const insertTarotReading = async (payload, timeoutMs = 8000) => {
  if (!restBase) return { data: null, error: new Error('Supabase URL missing') }
  const makeRequest = async () => {
    const headers = await buildAuthHeaders({
      'Content-Type': 'application/json',
...
export const deleteTarotReading = async (id, timeoutMs = 8000) => {
  if (!restBase) return { data: null, error: new Error('Supabase URL missing') }
  const makeRequest = async () => {
    const headers = await buildAuthHeaders()
...
export const selectHoroscopes = async (date, locale, timeoutMs = 8000) => {
  if (!restBase) return { data: null, error: new Error('Supabase URL missing') }
  const makeRequest = async () => {
    const headers = await buildAuthHeaders()
    const url =
...
export const invokeFunction = async (name, body, timeoutMs = 8000) => {
